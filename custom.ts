...
namespace custom {
    ...
    export function singlePoll(): void {
        ...
        // sendCmd(3);
        let cmd: number[] = [0xBB, 0x00, 0x03, 0x00, 0x01, 0x02, 0x06, 0x7E];
        let bufr: Buffer = pins.createBuffer(cmd.length);
        for (let idx = 0; idx < cmd.length; idx++) {
            bufr.setUint8(idx, cmd[idx])
        }
        serial.writeBuffer(bufr);
    ...
    export function multiPollStart(): void {
        ...
        serial.setRxBufferSize(32)
        // sendCmd(4);
        let cmd: number[] = [0xBB, 0x00, 0x27, 0x00, 0x03, 0x22, 0x27, 0x10, 0x83, 0x7E];
        let bufr: Buffer = pins.createBuffer(cmd.length);
        for (let idx = 0; idx < cmd.length; idx++) {
            bufr.setUint8(idx, cmd[idx])
        }
    ...
    export function multiPollEnd(): void {
        ...
        serial.setRxBufferSize(32)
        // sendCmd(5);
        let cmd: number[] = [0xBB, 0x00, 0x28, 0x00, 0x00, 0x28, 0x7E];
        let bufr: Buffer = pins.createBuffer(cmd.length);
        for (let idx = 0; idx < cmd.length; idx++) {
            bufr.setUint8(idx, cmd[idx])
        }
